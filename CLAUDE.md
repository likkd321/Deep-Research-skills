...
- **不要渲染各 item 的字段明细**——那部分密密麻麻、占据大量篇幅，是给机器看的中间产物；报告只呈现跨 item 的结论与关键数据，完整字段留在 `results/*.json` 里备查
...
### item 级研究深度：在 outline 阶段就定死

生成 items 时必须给每个 item 标 `research_depth`，决定派不派 agent、派谁：

| 档 | 谁来做 | 填什么 | 模型 |
|---|---|---|---|
| **详细** | 子 agent | 全部字段 | Sonnet 5 |
| **简要** | 子 agent | outline 指定的核心字段子集 | **Haiku 4.5** |
| **极简** | **主线程，不派 agent** | 3-5 个关键数字 + 信源 | — |

判断标准仍是「这个对象填满后结论会不会变」，但**不要只有"深调"和"砍掉"两档**——
国际对照、赛道旁支玩家通常属于「简要」：查 8-15 次、填 10 个左右关键字段就够，
比完全砍掉只靠已有知识写要扎实得多。

模型选择的硬约束：**Haiku 4.5 上下文窗口 200K，Sonnet 5 是 1M**。
一个「详细」档 item 要做 60-90 次检索，搜索结果全堆在同一上下文里，200K 装不下，
中途压缩或截断反而更贵。所以详细档只用 Sonnet，简要档才用 Haiku。

### field 级 detail_level：必须写进 prompt 才有用

fields.yaml 的 `极简/简要/详细` 默认是**死标签**——深调 prompt 不提它，
agent 就会对所有字段一视同仁地深挖。派 agent 时要显式要求它读取并据此分配检索预算：
...
### 判断型字段收归主线程，不要让子 agent 写

把字段分成两类，分工不同：

- **事实型**（MAU、时间线、定价、API 数量）→ 子 agent 检索填写
- **判断型**（`*_score`、胜率、优劣势、关键变量）→ **主线程写**

原因是结构性的：子 agent 并行且互相不可见，**不可能自发对齐口径**。
2026-09-18 那次 5 个 agent 各写了 9 个判断型字段，结果四家胜率相加远超 100%，
各自定义的「赢」还都不一样（有的按细分路线占位算，有的按三强格局算），
主线程几乎全部推翻重做——那 45 次判断型字段写作等于白花。

改成事实层/判断层分工后有三个收益，第三个最关键：

1. 省掉必然被丢弃的判断型写作
2. 结论一致性更好（横向校准本来就是主线程的活）
3. **解锁 Haiku**——挡住 Haiku 的原本是「要会打分」，这个要求拿掉后
   剩下的事实型字段就是检索+抽取，Haiku 完全胜任

fields.yaml 里仍然保留判断型字段的定义（报告要用），
只是由主线程在 synthesis 阶段填，不写进子 agent 的任务范围。

...
- **补查分工（按问题类型选执行者）**：
  - **item 内部的补深**（调和它自己见过的数字、澄清口径、再补几次搜索）→ 唤醒原子 agent 趁热补。它的上下文里有读过的来源、各来源口径和推算过程，改的是自己写的 JSON，质量高、前后一致，而且基本不用重搜，工具调用远少于从头调研。
  - **跨 item 的口径冲突**（两个 item 对同一数字给出不同值）或**与原调研关系不大的新问题** → 由主线程统一，或新开一个"复核 agent"集中处理：读相关 JSON、按统一口径核对后写回并重新验证。原 agent 各自只看过一边，分别唤醒既贵又难对齐；多个小问题合并给一个复核 agent，还能省掉每个 agent 的启动开销。
  - 原 agent 上下文已很大、缓存已冷，而问题又需要大量新搜索时，新开 agent 可能更省，按实际取舍。

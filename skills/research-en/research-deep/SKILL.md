...
- **Dispatch**: web/cloud sessions **MUST explicitly pass the model** when dispatching subagents (`model=sonnet` or `model=opus`; such environments do NOT read the `model` field in `agents/web-search-agent.md`); local CLI takes the default from that file's `model` field (sonnet) and overrides opus groups at dispatch.
